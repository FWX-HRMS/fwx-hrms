...
10. يلتزم الفريق الأول بتحصيل ودفع جميع مستحقات الفريق الثاني المتراكمة من العمل الإضافي والعمولات والبدلات والبدلات اليومية وفقاً لما تحدده ${companyDisplay}.
...
10. The First Party commits to collecting and paying to the Second Party all accrued overtime, commissions, allowances, and per diems as determined by ${companyDisplay}.

11. Financial Schedule:-
...
async function downloadContractPDF(kind, fileNumber, employeeName, text, signatureImage) {
  ...
  if (signatureImage) {
    ...
    if (!isArabicDoc) {
      doc.setFontSize(11);
      doc.setTextColor(27, 36, 48);
      const nameLine = `Employee Name: ${employeeName || "—"}`;
...
async function showDetails(id) {
  ...
  document.getElementById("detailsBody").innerHTML = `
    ...
    <div class="detail-row"><span class="label">${t("contractPeriodLabel")}</span><span class="value">${e.contract_period_months || "—"}</span></div>
